...
/**
 * Executes the db-tally queries, rather than merely parsing them.
 *
 * <p>Booting the JPA slice already validates every {@code @Query} string when the repository
 * beans are created, and {@code TallySchemaJpaTest} covers the DDL. Neither runs a statement.
...
@DisplayName("db-tally repositories")
class TallyRepositoryJpaTest {
    ...
    @Autowired private TallyGroupRepository groups;
    @Autowired private TallyGroupMemberRepository members;
    @Autowired private TallyExpenseRepository expenses;
    ...
    void reversalCancels() {
        ...
        TallyLedgerEntryEntity original = entry(g, a, b, "120.00");
        TallyLedgerEntryEntity reversal = entry(g, b, a, "120.00");
        reversal.setEntryType(TallyLedgerEntryType.REVERSAL);
    ...
    void entriesBySource() {
        ...
        String a = member(g, "A");
        String b = member(g, "B");
        TallyLedgerEntryEntity e1 = entry(g, a, b, "10.00");
        entry(g, a, b, "20.00").setSourceId(e1.getSourceId());
        em.flush();
        em.clear();

        assertThat(ledger.findBySourceTypeAndSourceId(TallyLedgerSourceType.EXPENSE, e1.getSourceId()))
                .hasSize(2);
    }
    ...
    void voidedExpensesAreExcluded() {
        ...
        em.find(TallyExpenseEntity.class, e).setStatus(TallyExpenseStatus.VOIDED);
        em.flush();
        em.clear();
    ...
    void keysetPaginationIsStable() {
        ...
        List<TallyExpenseEntity> page =
                expenses.findFirstPage(g, TallyExpenseStatus.ACTIVE, Limit.of(2));
        while (!page.isEmpty()) {
            page.forEach(x -> walked.add(x.getDescription()));
            TallyExpenseEntity last = page.getLast();
            page = expenses.findPageAfter(g, TallyExpenseStatus.ACTIVE,
    ...
    void clearInboundDelegations() {
        ...
        // rewritten as `where paid_for_by_member_id in (select id from tally_group_member ...)`
        ...
        assertThat(em.find(TallyGroupMemberEntity.class, kid1).getPaidForByMemberId()).isNull();
    }
    ...
    void clearInboundDelegationsIsGroupScoped() {
        ...
        assertThat(em.find(TallyGroupMemberEntity.class, kid2).getPaidForByMemberId())
                .isEqualTo(appa2);
    }
    ...
    void ghostClaimRepointsEveryReference() {
        ...
        // uk_tally_group_member_group_user.
        String g = group();
        String ghost = member(g, "Amma (ghost)");
        String real  = member(g, "Amma");
        em.find(TallyGroupMemberEntity.class, real).setUserId(42L);
        ...
        assertThat(payers.findByExpenseId(e)).extracting(TallyExpensePayerEntity::getMemberId)
                .containsOnly(real);
        assertThat(shares.findByExpenseId(e))
                .extracting(TallyExpenseShareEntity::getBeneficiaryMemberId,
                            TallyExpenseShareEntity::getOwedByMemberId)
        ...
        assertThat(settlements.findByGroupIdAndStatusOrderBySettledAtDesc(g, TallySettlementStatus.ACTIVE))
    ...
    void rosterKeepsHistory() {
        ...
        em.find(TallyGroupMemberEntity.class, left).setStatus(TallyMemberStatus.LEFT);
        em.find(TallyGroupMemberEntity.class, left).setUserId(9L);
        em.flush();
        em.clear();

        // Unfiltered, so an old expense can still render "Left" as a name.
        assertThat(members.findByGroupId(g)).extracting(TallyGroupMemberEntity::getId)
                .containsExactlyInAnyOrder(stayed, left);
        assertThat(members.findByGroupIdAndStatus(g, TallyMemberStatus.ACTIVE))
                .extracting(TallyGroupMemberEntity::getId).containsExactly(stayed);
        assertThat(members.findByUserIdAndStatus(9L, TallyMemberStatus.ACTIVE)).isEmpty();
    }
    ...
    void archivedGroupIsClosedForWrites() {
        ...
        em.find(TallyGroupEntity.class, g).setArchivedAt(Instant.now());
        em.flush();
        em.clear();
    ...
    void idempotencyIsPerGroup() {
        String g1 = group();
        String g2 = group();
        TallyExpenseEntity a = em.find(TallyExpenseEntity.class,
                expense(g1, "First", "10.00", LocalDate.of(2026, 9, 1)));
        a.setIdempotencyKey("retry-1");
        TallyExpenseEntity b = em.find(TallyExpenseEntity.class,
        ...
        assertThat(expenses.findByGroupIdAndIdempotencyKey(g1, "retry-1"))
                .get().extracting(TallyExpenseEntity::getDescription).isEqualTo("First");
        assertThat(expenses.findByGroupIdAndIdempotencyKey(g2, "retry-1"))
                .get().extracting(TallyExpenseEntity::getDescription).isEqualTo("Second");
    }
    ...
    private void delegate(String memberId, String toMemberId) {
        em.find(TallyGroupMemberEntity.class, memberId).setPaidForByMemberId(toMemberId);
    }

    private String expense(String groupId, String description, String total, LocalDate date) {
        TallyExpenseEntity e = new TallyExpenseEntity();
    ...
    private TallyLedgerEntryEntity entry(String groupId, String from, String to, String amount) {
        TallyLedgerEntryEntity e = new TallyLedgerEntryEntity();
        e.setGroupId(groupId);
        e.setFromMemberId(from);
        e.setToMemberId(to);
        e.setAmount(new BigDecimal(amount));
        e.setSourceType(TallyLedgerSourceType.EXPENSE);
    ...
    private static Map<String, BigDecimal> totals(List<TallyLedgerEntryRepository.MemberTotal> rows) {
        return rows.stream().collect(Collectors.toMap(
                TallyLedgerEntryRepository.MemberTotal::getMemberId,
