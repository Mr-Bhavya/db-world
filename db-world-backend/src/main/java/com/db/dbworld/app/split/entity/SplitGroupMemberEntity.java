 ...
 */
@Entity
@Table(name = "split_group_member", schema = "db_world",
        uniqueConstraints = {
            ...
            @UniqueConstraint(name = "uk_split_group_member_group_user",
        ...
        indexes = {
            ...
            @Index(name = "idx_split_group_member_user", columnList = "user_id, group_id"),
            // Roster read, and the removal sweep that clears inbound delegations.
            @Index(name = "idx_split_group_member_group", columnList = "group_id, status"),
            @Index(name = "idx_split_group_member_delegate", columnList = "paid_for_by_member_id")
        })
@Getter @Setter @NoArgsConstructor
public class SplitGroupMemberEntity {
    ...
    private SplitMemberRole role = SplitMemberRole.MEMBER;

    @Enumerated(EnumType.STRING)
    @Column(nullable = false, length = 20)
    private SplitMemberStatus status = SplitMemberStatus.ACTIVE;
    ...
    public boolean isActive() {
        return status == SplitMemberStatus.ACTIVE;
    }
}
