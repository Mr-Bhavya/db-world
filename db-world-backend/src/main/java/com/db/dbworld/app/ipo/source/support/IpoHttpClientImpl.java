    ...
    public IpoHttpResponse get(String url, Map<String, String> headers) {
        try {
            ResponseEntity<String> entity = webClient.get()
                    ...
                    // body read — same rationale as TmdbClient's use of transientNetworkRetry.
                    .retryWhen(IpoWebClientConfig.transientNetworkRetry(url))
                    .block(REQUEST_TIMEOUT);

            if (entity == null) {
                throw new SourceFetchException("Null response for GET " + url);
            }
            return new IpoHttpResponse(entity.getStatusCode().value(), entity.getBody(), entity.getHeaders());
