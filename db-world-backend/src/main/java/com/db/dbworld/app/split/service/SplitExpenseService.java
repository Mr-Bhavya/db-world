...
public class SplitExpenseService {
    ...
    private final SplitAccessService access;
    private final SplitLedgerService ledgerService;
    private final SplitExpenseRepository expenses;
    private final SplitExpensePayerRepository payers;
    private final SplitExpenseShareRepository shares;
     ...
     */
    @Transactional
    public SplitExpenseEntity create(Long userId, String groupId, CreateExpenseRequest request) {
        ...
        Map<String, SplitGroupMemberEntity> roster = members.findByGroupIdAndStatus(groupId, SplitMemberStatus.ACTIVE)
                .stream().collect(Collectors.toMap(SplitGroupMemberEntity::getId, Function.identity()));
        ...
        List<SplitExpensePayerEntity> payerRows = buildPayers(expense, request.payers());
        List<SplitExpenseShareEntity> shareRows = buildShares(expense, request, roster);
     ...
     */
    @Transactional
    public SplitExpenseEntity voidExpense(Long userId, String expenseId) {
        SplitExpenseEntity expense = expenses.findById(expenseId)
        ...
        expense.setStatus(SplitExpenseStatus.VOIDED);
        ledgerService.reverse(SplitLedgerSourceType.EXPENSE, expense.getId());
        return expense;
    }
     ...
     */
    @Transactional
    public SplitExpenseEntity replace(Long userId, String expenseId, CreateExpenseRequest request) {
        SplitExpenseEntity original = expenses.findById(expenseId)
                .orElseThrow(() -> new DbWorldException(HttpStatus.NOT_FOUND, "Expense not found"));
        ...
        CreateExpenseRequest replacement = request.idempotencyKey() == null ? request
                : new CreateExpenseRequest(request.description(), request.totalAmount(),
                        request.splitMethod(), request.category(), request.expenseDate(),
    ...
    private List<SplitExpensePayerEntity> buildPayers(SplitExpenseEntity expense, List<PayerInput> inputs) {
        Set<String> seen = new HashSet<>();
        List<SplitExpensePayerEntity> rows = new ArrayList<>(inputs.size());
     ...
     *
     * <p>All four methods go through {@link SplitAllocator} — including EXACT, which might look
     ...
     */
    private List<SplitExpenseShareEntity> buildShares(SplitExpenseEntity expense,
                                                      CreateExpenseRequest request,
                                                      Map<String, SplitGroupMemberEntity> roster) {
        ...
        List<SplitAllocator.Allocation> amounts = switch (request.splitMethod()) {
            case EQUAL -> SplitAllocator.allocateEqually(total,
            ...
            case EXACT -> {
                ...
                yield participants.stream()
                        .map(p -> new SplitAllocator.Allocation(p.memberId(), p.exactAmount()))
                        .toList();
            }
            ...
            case PERCENT -> {
                ...
                yield SplitAllocator.allocate(total, participants.stream()
                        .map(p -> new SplitAllocator.Weight(p.memberId(), p.percent()))
                        .toList());
            }
        ...
        Map<String, BigDecimal> byMember = amounts.stream()
                .collect(Collectors.toMap(SplitAllocator.Allocation::memberId, SplitAllocator.Allocation::amount));

        List<SplitExpenseShareEntity> rows = new ArrayList<>(participants.size());
        for (ParticipantInput p : participants) {
    ...
    private static List<SplitAllocator.Allocation> yieldShares(BigDecimal total, List<ParticipantInput> participants) {
        List<SplitAllocator.Weight> weights = participants.stream()
                .map(p -> new SplitAllocator.Weight(p.memberId(),
                        requirePresent(p.shareWeight(), "a number of shares", p.memberId())))
                .toList();
        return SplitAllocator.allocate(total, weights);
    }
     ...
     * they were bought.
     */
    private String resolveLiability(ParticipantInput p, Map<String, SplitGroupMemberEntity> roster) {
    ...
    private static void requireKnownMembers(Map<String, SplitGroupMemberEntity> roster,
                                            CreateExpenseRequest request) {
