 ...
 */
@Entity
@Table(name = "split_expense_share", schema = "db_world",
        uniqueConstraints = {
            ...
            @UniqueConstraint(name = "uk_split_expense_share_expense_beneficiary",
                    columnNames = {"expense_id", "beneficiary_member_id"})
        },
        indexes = {
            @Index(name = "idx_split_expense_share_expense", columnList = "expense_id"),
            // "What am I liable for" reads by owed_by, which the unique key above cannot serve.
            @Index(name = "idx_split_expense_share_owed", columnList = "owed_by_member_id")
        })
@Getter @Setter @NoArgsConstructor
public class SplitExpenseShareEntity {
