...
import com.db.dbworld.app.split.entity.SplitLedgerEntryEntity;
import com.db.dbworld.app.split.entity.SplitLedgerSourceType;
...
public interface SplitLedgerEntryRepository extends JpaRepository<SplitLedgerEntryEntity, String> {
     ...
     * (idx_split_ledger_entry_group_to / _group_from) and is answered from the index without
     ...
     */
    @Query("""
            select coalesce((select sum(c.amount) from SplitLedgerEntryEntity c
                              where c.groupId = :groupId and c.toMemberId = :memberId), 0)
                 - coalesce((select sum(d.amount) from SplitLedgerEntryEntity d
    ...
    List<SplitLedgerEntryEntity> findBySourceTypeAndSourceId(SplitLedgerSourceType sourceType,
                                                             String sourceId);

    List<SplitLedgerEntryEntity> findByGroupId(String groupId);
     ...
     *   1. split_expense_payer.member_id
     *   2. split_expense_share.beneficiary_member_id
     *   3. split_expense_share.owed_by_member_id
     *   4. split_ledger_entry.from_member_id            <- here
     *   5. split_ledger_entry.to_member_id              <- here
     *   6. split_settlement.from_member_id
     *   7. split_settlement.to_member_id
     *   8. split_group_member.paid_for_by_member_id     <- self-referencing, the forgotten one
     */

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("update SplitLedgerEntryEntity e set e.fromMemberId = :survivorId where e.fromMemberId = :loserId")
    int repointFrom(@Param("survivorId") String survivorId, @Param("loserId") String loserId);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("update SplitLedgerEntryEntity e set e.toMemberId = :survivorId where e.toMemberId = :loserId")
