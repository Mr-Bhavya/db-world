...
FROM   db_world.rails r
WHERE  NOT EXISTS (
    SELECT 1 FROM db_world.rails_page_types pt WHERE pt.rail_id = r.id
);
...
INSERT IGNORE INTO db_world.rails_page_types (rail_id, page_type)
SELECT id, COALESCE(page_type, 'HOME')
FROM   db_world.rails
WHERE  NOT EXISTS (
    SELECT 1 FROM db_world.rails_page_types pt WHERE pt.rail_id = rails.id
);

-- ---- 3. Drop the legacy column ---------------------------------------------
ALTER TABLE db_world.rails DROP COLUMN page_type;

-- ---- 4. Verify --------------------------------------------------------------
-- Should NOT include page_type.
SHOW COLUMNS FROM db_world.rails;
