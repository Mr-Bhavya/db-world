...
export default function RecordCreateModal({ open, onClose }) {
  ...
  const handleQueryChange = (e) => {
    ...
    searchTimer.current = setTimeout(() => doSearch(v, typeValue), 500);
  };
  ...
  const handleClose = () => {
    setQuery(''); setResults([]); setSelected(null); setSearchError('');
    reset({ type: 'MOVIE', tmdbId: '' });
    onClose();
  };
    ...
    <Dialog open={open} onClose={handleClose} PaperProps={{ sx:{ bgcolor:'#0d0d18', border:'1px solid rgba(255,255,255,0.08)', color:'#fff', width:'100%', maxWidth:580 } }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <DialogActions sx={{ px:3, pb:2 }}>
          ...
          <Button type="submit" disabled={!selected || isPending} variant="contained" sx={{ bgcolor:'#6366f1','&:hover':{ bgcolor:'#5254cc' } }}>
