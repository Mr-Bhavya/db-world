    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (bearerToken != null && bearerToken.startsWith("Bearer")) {
            ...
            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                logger.info("fetched User details. User: {}", userDetails.getUsername());

                Boolean isTokenValidate = this.jwtHelper.validateToken(token, userDetails);
                logger.info("Token Validate :  {}", isTokenValidate);
                if (isTokenValidate) {
                    //set the authentication
                    logger.info("Setting authentication for User: {} and authority is {} ", userDetails.getUsername(), userDetails.getAuthorities());
